...
- Android 详情页在带状态栏安全区的窄屏上验证标题不与系统图标重叠；阅读器进入、控制层显隐和前台恢复都必须请求“仅保留底部覆盖层”的系统 UI 模式，离开后恢复 `edgeToEdge`，且不得请求 `immersiveSticky`。正文测试同时断言短段和自动换行长段的首个正文字符位于同一两字缩进位置、正文保持两端对齐，并覆盖换行续行与跨页续行不重复缩进。
