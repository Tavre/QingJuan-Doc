...
- 当前基线：`2.3.0+44`，后续 build 必须 > `44`，不得回退 / 复用。该功能版加入书库管理、连载追更、阅读标注与离线缓存、备份恢复、存储配额、插件维护和翻译质量管理，对外标签 `v2.3.0`。
...
5. Android 分别验证 PC 局域网扫码与 Linux 首次远程连接；Windows 验证本机后端启动 + Linux 远程连接，覆盖导入、任务、阅读、设置、切换、退出；
...
- Android 忽略旧 `local` 偏好且 APK 不内置后端；用户可显式填写 Linux 地址与 Token，也可扫描 Windows 客户端生成的 PC 局域网连接二维码。连接来源必须清楚可见，失败时不得静默切换。
