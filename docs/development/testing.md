...
Widget 测试需要分别覆盖 Android 五入口底部导航与 Windows `NavigationPane`，证明窗口宽度变化不会让两种平台界面互相切换；同时覆盖详情页状态栏安全区和完整简介滚动、手机文本输入关闭放大镜但保留选区、Token 显式粘贴、小说段落两字缩进与两端对齐、阅读器顶部状态栏、分页及连续模式音量键操作、窄屏搜索导入、共享移动端底部面板的键盘与关闭行为，以及加载/空/错误状态、长中文标题、文字缩放、深色主题和减少动态效果。
...
抓取器测试使用固定 Fixture 或 Mock，不把不稳定公网作为 CI 前置条件。API 测试必须覆盖认证、路径边界、导入导出、任务恢复、22 个站点插件的注册与禁用拦截，以及敏感信息不泄露。搜索测试还要覆盖稳定来源 ID、切换引擎清空旧结果、夸克目录短时复用、限流及 502 / 503 / 504 有界重试、免费完整章节边界、拷贝漫画分组目录，以及 COMICORES 无章节能力。账号插件还要覆盖不透明扫码流程 ID、会话失效、分组 / 分页去重、已有作品跳过、并发任务拒绝和单本失败隔离。Linux 章节缓存测试需要覆盖服务级任务恢复、同章合并、阅读抢占、单章失败隔离和当前章响应前的下一章预取；番茄测试还要覆盖 HTTP 200 空页、验证页和缺少 `__INITIAL_STATE__` 的有界重试。
