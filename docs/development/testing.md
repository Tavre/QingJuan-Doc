...
抓取器测试使用固定 Fixture 或 Mock，不把不稳定公网作为 CI 前置条件。API 测试必须覆盖认证、路径边界、导入导出、任务恢复、站点插件注册与禁用拦截，以及敏感信息不泄露。账号插件还要覆盖不透明扫码流程 ID、会话失效、分组 / 分页去重、已有作品跳过、并发任务拒绝和单本失败隔离。
...
v1.6.0 的发布级验证还应在可访问的真实网络中分别检查番茄与起点扫码登录和账号书架导入。若目标站点 TLS、地区或网络条件不可用，只能报告该项未验证，不能以 Mock 测试替代真实登录结论。
