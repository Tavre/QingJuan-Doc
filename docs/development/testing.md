...
Widget 测试需要分别覆盖 Android“书库、推荐、搜索、任务、我的”五入口导航与 Windows 桌面 Shell，证明窗口宽度变化不会让两种平台界面互相切换；同时覆盖详情与阅读器、局域网二维码和连接链接、在线更新、禁漫本子号导入、导入插件搜索，以及加载/空/错误状态、长中文标题、文字缩放、深色主题和减少动态效果。
...
`2.1.1+41` 修复 Android 真实路由的文字样式继承、加载语义与窄屏操作布局。`2.2.0+42` 完成在线更新、安装器、PC 扫码连接与插件包发布；`2.2.1+43` 覆盖 Fake-IP 和加密 DNS 回退。`2.3.0+44` 还需覆盖书库元数据与追更、阅读标注与离线缓存、备份恢复、文件配额、公平任务调度、插件回退和翻译质量管理。
