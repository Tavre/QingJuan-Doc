...
- 当前已发布版本：`2.1.0+40`，对外版本为 `v2.1.0`。2.1.0 完成移动端重设计，并新增 Windows 漫画翻译工作台、书架译文写回与系统托盘体验；Linux 后端多用户、账号安全和 Windows 本机单用户模式继续兼容。
...
- 既有功能基线包括：本地文件上传导入、单章与多章导出、设备 TTS、链接任务与实时日志、单一 OpenAI 兼容翻译配置、RapidOCR 漫画翻译链路，以及 Windows 九模式漫画翻译和三栏文本编辑工作台；v2.1.0 加入按章节完整性校验的书架译文写回。
