...
- Android 使用触控优先的移动端标题、底部导航和沉浸式阅读界面；
- 调整窗口宽度只会改变当前平台内部的排版，不会把 Windows 界面切换成 Android 布局。

v1.6.0 将 20 个内置站点解析器组织为可独立启停的插件，并为番茄与起点加入账号登录和书架导入。Windows 本机模式在客户端 **插件配置** 中管理；Linux 远程模式在服务器管理界面的 **插件管理** 中管理，Android 不显示本地插件入口。完整清单见[内置解析器](./parsers/)。
