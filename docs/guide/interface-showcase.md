...
青卷在 Windows 与 Android 上共享搜索、下载、翻译、阅读和听书能力，同时针对键鼠宽屏与手机触控分别设计界面。下面的截图来自 v1.7.6，用于展示两端的基础布局；v2.3.0 的 Android 当前使用“书库、推荐、搜索、任务、我的”五入口界面。因此移动端截图主要用于对照基础功能，导航与视觉细节以当前客户端为准。
...
Android 客户端针对单手触控、安全区与不同屏幕比例设计。v2.3.0 的手机使用紧凑液态胶囊底栏组织“书库、推荐、搜索、任务、我的”，平板使用移动侧栏和双栏内容；Android 不包含本机后端，可扫描二维码连接同一局域网内的 PC，也可连接已部署的 Linux 后端并登录用户账号。
