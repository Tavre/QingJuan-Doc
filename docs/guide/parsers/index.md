...
青卷 v1.7.0 起随后端提供 22 个内置站点解析器。每个解析器都有独立的稳定 ID、匹配域名、作品类型、能力和启停状态；它们与用户导入的 Legado / 阅读 App JSON 书源相互独立。v1.7.1 至 v1.7.4 没有新增解析器，主要增强夸克导入、Linux 章节缓存、番茄正文重试与客户端阅读体验。
