...
本页的安装包文件名按当前正式版 `v2.1.0` 编写。Windows x64 ZIP、Android APK 与两份 SHA-256 校验文件由正式发布工作流生成。使用远程模式时，客户端与服务端应同时升级到 v2.1.0，以使用漫画书架写回、移动端新界面及当前 API 行为。
...
```powershell
Get-FileHash .\QingJuan-v2.1.0-windows-x64.zip -Algorithm SHA256
Get-Content .\QingJuan-v2.1.0-windows-x64.zip.sha256
```

Android APK 也可以在电脑上校验：

```powershell
Get-FileHash .\QingJuan-v2.1.0-android.apk -Algorithm SHA256
Get-Content .\QingJuan-v2.1.0-android.apk.sha256
```
