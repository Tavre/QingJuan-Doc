...
6. 生成连接 Token、管理密码、管理会话密钥和独立的 2FA 加密密钥；
7. 安装后端服务和受控的 systemd 在线升级器；
8. 启动服务并等待本机健康检查；
9. 输出客户端地址、管理地址和首次管理密码。
...
Linux 后端为多用户服务。客户端先填写 FastAPI 地址和连接 Token，再在 **我的** 中注册或登录；管理界面的 **用户管理**、**注册设置** 和 **后端升级** 用于维护账号、注册条件及服务更新。Windows 本机模式和 v2.2.0 的 Android 局域网连接 PC 均不需要本页部署。
