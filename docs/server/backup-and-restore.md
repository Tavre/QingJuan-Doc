...
v2.3.0 提供受控的完整书库备份、校验和恢复，也支持将 Windows 本机书库显式迁移到空的 Linux 服务端。服务器级灾难恢复仍应配套保管 `/var/lib/qingjuan` 与 `/etc/qingjuan/backend.env`。

## 应用内备份与恢复

Linux 管理员可在 **备份管理** 创建、校验、下载、恢复和删除备份；Windows 本机模式可从客户端进入相同流程。备份包含数据库、作品文件、插件包、笔记、进度和功能状态，但不包含后端环境文件中的管理密码、连接 Token、模型密钥或 2FA 加密密钥。

恢复前青卷会检查归档格式、摘要、路径、账号归属和版本兼容性，并暂停新的业务写入。恢复失败或进程中断时保留恢复状态和原数据，不会把半套文件作为成功结果。执行前仍应下载并另存一份已校验备份。

Windows 本机书库可以显式上传并迁移到一个空的 Linux 服务端账号。该操作不会持续同步两端；迁移完成后应在目标账号核对书架、章节、译文、笔记和进度，再决定是否清理原本机数据。

## 手工灾难恢复备份

青卷不承诺操作系统级无停机复制。可靠做法是短暂停止后端，配套复制 `/var/lib/qingjuan` 与 `/etc/qingjuan/backend.env`，再启动服务。
