...
v1.7.0 源码候选包含 22 个可启停解析插件：Windows 本机模式在客户端 **插件配置** 中管理；Linux 远程模式在 `/admin/#plugins` 管理；Android 由服务器管理员统一维护。番茄与起点支持账号登录和书架导入；夸克仅处理匿名免费完整章节；COMICORES 只提供公开搜索和元数据，不提供章节。插件随版本发布，目前不支持在线安装、卸载或上传任意代码。
